...
class UASocketProtocol(asyncio.Protocol):
    ...
    def _send_raw_request(self, body, timeout: float = 1, message_type=ua.MessageType.SecureMessage) -> asyncio.Future:
        """
        Send raw OPCUA request to server, lower-level method.

        The raw request MUST NOT include the OPCUA RequestHeader. It
        will be automatically generated and inserted after the first four
        bytes corresponding to the request type (NodeID). The request
        parameters follow right after the request type.
        ...
        # Basically, we create an empty RequestHeader and ask the UA client
        # to fill its fields with the expected values (authentication token,
        # request ID, etc.), so we don't have to care about it.
        header = ua.RequestHeader()
        self._setup_request_header(header, timeout)

        try:
            # We rebuild the raw request by extracting its type, adding the
            # generated header and appending the request parameters (body).
            binreq = body[:4] + struct_to_binary(header) + body[4:]
        except Exception:
            # reset request handle if any error
            # see self._setup_request_header
            self._request_handle -= 1
        ...
        # We then convert this secure message into a binary-encoded message,
        # this method will handle any encryption/signature step if required.
        msg = self._connection.message_to_binary(binreq, message_type=message_type, request_id=self._request_id)
        if self.transport is not None:
            # Send the resulting bytes into the connection
            self.transport.write(msg)
        return future
    ...
    async def send_raw_request(self, body, timeout: Optional[float] = None, message_type=ua.MessageType.SecureMessage):
        """
        ...
        if self.pre_request_hook:
            ...
            await self.pre_request_hook()
        try:
            data = await asyncio.wait_for(self._send_raw_request(body, timeout, message_type), timeout if timeout else None)
...
class UaClient(AbstractSession):
    ...
    async def send_raw(self, raw_body: bytes):
        """Send a raw request (secure message) and return response
        """
        self.logger.info("send raw request")
        data = await self.protocol.send_raw_request(raw_body, timeout=5.0)
        return data
