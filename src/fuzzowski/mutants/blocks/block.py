...
class Block(Mutant):
    ...
    def render(self, replace_node: str = None, replace_value: bytes = None, original: bool = False) -> bytes:
        ...
        # Original Fuzzowski comment below ... they definitely don't know what they were doing :/
        #
        ...
        if self.dep:
            # Call dependency check, if it returns None then dependency value is valid and
            # we can continue. If it returns some bytes, dependency condition is not met
            # and we render as an empty block.
            result = self._render_dep()
